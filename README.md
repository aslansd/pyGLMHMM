...
- First, an instance of `GLMHMMEstimator` class must be made with its different parameters. The most important ones are:
...
- Second, the `GLMHMMEstimator.fit(X, y, [])` method must be run on the input data:
  ...
  - `y` (symb): A list with the length of the `num_samples`.  Each element of the list is a one-dimensional numpy array with the size of ((number of time points)). They represent the emitted bahaviors or actions (like song types) across time and they match up with the corresponding element of the `X` input.
